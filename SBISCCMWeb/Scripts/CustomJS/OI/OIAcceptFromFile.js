$("body").on('change', '#MatchAcceptFile', function () {
    if ($('#file').val() != "") {
        ...
        if ($.inArray($(this).val().split('.').pop().toLowerCase(), fileExtension) == -1) {
            ...
            ShowMessageNotification("success", matchAcceptAllowedFormats + fileExtension.join(', '), true);
            $('#MatchAcceptFile').val("");
        }
        else {
...
$("body").on('click', '#btnInsertAcceptData', function () {
    ...
    if ($("#DataColumn-0").val() == 0 || $("#DataColumn-1").val() == 0 || $("#DataColumn-2").val() == 0) {
        ...
        $.ajax({
            type: "POST",
            url: "/OIMatchData/AcceptDataFile",
            ...
            success: function (data) {
                ...
                $("#OIMatchDataAcceptFromFileModal").modal("hide");
...
function LoadAcceptMapping(formData) {
    $.ajax({
        type: "POST",
        url: '/OIMatchData/BindAcceptFileMapping',
        data: formData,
        dataType: 'html',
        async: false,
        contentType: false,
...
function LoadOIMatchAcceptFromFile() {
    var cnt = 0;
    if (parseInt($("#DataColumn-0").val()) > 0 && parseInt($("#DataColumn-1").val()) > 0 && parseInt($("#DataColumn-2").val()) > 0) {
    }
    else {
