...
$('body').on('click', '.filenameCheck', function () {
    ...
    var SrcRecID = $("#tempSrcRecID").val().replace(":", "");
...
$('body').on('click', '#btnSubmitExportName', function () {
    ...
    $.ajax({
        type: "POST",
        url: "/ExportView/IsExistExportFileName?Parameters=" + ConvertEncrypte(zipFileName).split("+").join("***"),
        ...
        success: function (data) {
            ...
            if (data == notExist) {
                parent.ExportSubmitProcess(zipFileName);
            }
            else if (data == exist) {
                ...
                ShowMessageNotification("success", fileNameExists, false);
            }
            else {
                //show notification message instade of Bootbox Message...if you pass "success" than show Notification message and if you pass "error" than show bootbox message
