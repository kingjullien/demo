...
//Create Custom function :- collapse and expand menu with show icon on collapse
//Set or Get Custom localStorage
function MinifyMenu() {
    $('body').toggleClass(function (e) {
        if ($(this).is(".minified")) {
            $(".circleUserText").show();
            $(".txtCompliance").show();
            $(".Toggleslider").removeClass("clsComplianceMinified");
            $(".Toggleswitch").addClass("switch");
            $(".Toggleswitch").removeClass("switchADACompliance");
            localStorage.setItem("minifyme", true);
            // Set for provider menu when Collapse/Expand 
            $(".divCustomeProviderSubmenu").removeClass("minifymeProviderMenu");
            return "minified";
        } else {
            localStorage.setItem("minifyme", false);
            // Set for provider menu when Collapse/Expand 
            $(".divCustomeProviderSubmenu").addClass("minifymeProviderMenu");
            $(".circleUserText").hide();
            $(".txtCompliance").hide();
            $(".Toggleslider").addClass("clsComplianceMinified");
            $(".Toggleswitch").removeClass("switch");
            $(".Toggleswitch").addClass("switchADACompliance");
...
$(document).ready(function () {
    ...
    //Set or Get Custom localStorage
    var minifyme = localStorage.getItem("minifyme");
    if (minifyme != null && minifyme.toLocaleLowerCase() == "false") {
        MinifyMenu()
    }
...
function setup_dashboard_widgets_desktop() {

    if ($.fn.jarvisWidgets && $.enableJarvisWidgets) {

        $('.widget-data').jarvisWidgets({
...
//Notification For
$('#NotifyNotification').click(function (e) {
    var $this = $(this);
    if ($this.find('.badge').hasClass('bg-color-red')) {
        $this.find('.badge').removeClassPrefix('bg-color-');
        $this.find('.badge').text("0");
    ...
    if (!$this.next('.ajax-dropdown').is(':visible')) {
        ...
        $("#NotificationDiv .Notification-Containt li").each(function () {
            if ($(this).attr("data-isRead").toLowerCase() == "false") {

                temp.push($(this).attr("data-notificationid"));
            }
        });
        if ($("#NotifyNotification b.badge.bounceIn.animated").length == 0) {
            return false;
        }
        var NotificationId = temp.join(',');
        $.ajax({
            type: "POST",
            url: "Notification/DismissNotification?NotificationId=" + NotificationId,
            ...
            success: function (response) {
                if (response = "success") {
                    $("#NotifyNotification b.badge.bounceIn.animated").remove()
                }
